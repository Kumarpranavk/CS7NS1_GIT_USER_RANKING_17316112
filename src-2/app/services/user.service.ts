...
export class UserService {
    ...
    getUserCommits(user: string): Observable<any> {

        let endPoint = '/users/' + user + '/repos/commits';
        return this.api.get(endPoint).map(res => res.json()).catch(err => Observable.throw(err));
    }

    getUserCommits2018(user: string): Observable<any> {

        let endPoint = '/users/' + user + '/repos/commits/{year}';
        return this.api.get(endPoint).map(res => res.json()).catch(err => Observable.throw(err));
    }
